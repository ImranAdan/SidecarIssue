...
public class API {
    ...
    public String invokeJavaService2(@RequestParam(name = "message", required = true) String message){
    	return javaService.testPost(message);
    }
    
    @RequestMapping(value = "/testPythonService1", method = RequestMethod.GET)
    public String invokePythonService(@RequestParam(name = "message", required = true) String message){
    	return pythonService.testPostContentTypeJson(message);
    }
    
    @RequestMapping(value = "/testPythonService2", method = RequestMethod.GET)
    public String invokePythonService2(@RequestParam(name = "message", required = true) String message){
    	return pythonService.testPostContentTypeWWWForm(message);
    }
    
    @RequestMapping(value = "/testPythonService3", method = RequestMethod.GET)
    public String invokePythonService3(@RequestParam(name = "message", required = true) String message){
